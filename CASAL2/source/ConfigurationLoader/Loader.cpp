 ...
 */
bool Loader::LoadConfigFile(GlobalConfiguration& global_config, const string &override_file_name) {
	...
	if (!skip_loading_file) {
		...
		if (!file.OpenFile(config_file))
			LOG_FATAL()
			<< "Failed to open the first configuration file: " << config_file << ". Does this file exist? Is it in the right path?";
		...
		if (file_lines_.size() == 0)
			LOG_FATAL()
			<< "The configuration file " << config_file << " is empty. Please specify a valid configuration file";
	} else {
...
void Loader::ParseBlock(Model* model, vector<FileLine> &block) {
	...
	if (line_parts.size() == 0)
		LOG_FATAL()
		<< "At line " << block[0].line_number_ << " in " << block[0].file_name_
	...
	for (FileLine file_line : block) {
		...
		if (line.length() >= 5 && line.substr(0, 4) == PARAM_TYPE) {
			...
			if (line_parts.size() == 0)
				LOG_FATAL()
				<< "At line " << file_line.line_number_ << " in " << file_line.file_name_
						<< ": Could not successfully split the line into an array. Line is incorrectly formatted";

			if (line_parts.size() != 2)
				LOG_FATAL()
				<< "At line " << file_line.line_number_ << " in " << file_line.file_name_ << ": No valid value was specified as the type";
			...
			if (line_parts.size() != 2)
				LOG_FATAL()
				<< "At line " << file_line.line_number_ << " in " << file_line.file_name_ << ": No valid value was specified as the partition_type";

			string temp = utilities::ToLowercase(line_parts[1]);
			if (!utilities::To<PartitionType>(temp, partition_type))
				LOG_FATAL()
				<< "Could not convert value " << temp << " to a valid partition_type (model, age, length, hybrid)";
	...
	if (!object)
		LOG_FATAL()
		<< "At line " << block[0].line_number_ << " in " << block[0].file_name_ << ": Block object type or sub-type is invalid.\n" << "Object Type: "
				<< block_type << "\n" << "Sub-Type: " << sub_type;

	if (block_label != "" && !object->parameters().Add(PARAM_LABEL, block_label, block[0].file_name_, block[0].line_number_))
		LOG_FATAL()
		<< "At line " << block[0].line_number_ << " in " << block[0].file_name_ << ": The block @" << block_type << " does not support having a label";
...
//	}
	...
	for (FileLine file_line : block) {
		...
		if (!loading_table && parameter_type == PARAM_TABLE) {
			...
			if (!current_table_)
				LOG_FATAL()
				<< "At line " << file_line.line_number_ << " in " << file_line.file_name_ << ": table " << table_label << " is not a supported table label.";
			...
			if (current_table_->requires_comlums() && values.size() != current_table_->column_count()) {
				LOG_FATAL()
				<< "At line " << file_line.line_number_ << " in " << file_line.file_name_ << ": Table data does not contain the correct number of columns. Expected ("
			...
			if (parameter_type != PARAM_PARAMETER && parameter_type != PARAM_EQUATION && !util::String::HandleOperators(values, error))
				LOG_FATAL()
				<< "At line " << file_line.line_number_ << " in " << file_line.file_name_ << ": " << error;
			...
			if (!object->parameters().ignore_all_parameters()) {
				...
				if (!parameter) {
					...
					LOG_ERROR() << "At line " << file_line.line_number_ << " in " << file_line.file_name_ << ": Parameter '" << parameter_type << "' for object "
							<< block_type << " was already specified at line " << parameter->line_number() << " in " << parameter->file_name();
				} else if (!object->parameters().Add(parameter_type, values, file_line.file_name_, file_line.line_number_)) {
					LOG_ERROR() << "At line " << file_line.line_number_ << " in " << file_line.file_name_ << ": Could not add parameter '" << parameter_type
							<< "' to block '" << block_type << "'. Parameter is not supported";
				}
	...
	if (first_inline_bracket != string::npos && second_inline_bracket != string::npos) {
		...
		while (first_inline_bracket != string::npos) {
			if (first_inline_bracket >= second_inline_bracket)
				LOG_CODE_ERROR()
				<< "first_inline_bracket (" << first_inline_bracket << ") <= second_inline_bracket (" << second_inline_bracket << ")";
			if (first_inline_bracket <= 1)
				LOG_FATAL()
				<< "At line " << file_line.line_number_ << " in " << file_line.file_name_ << ": You cannot start the line with an inline definition [ operator";
			...
			if (file_line.line_[first_inline_bracket - 1] == '=') {
				...
				if (space_loc == string::npos)
					LOG_CODE_ERROR()
					<< "space_loc == string::npos for line: " << start_string;
